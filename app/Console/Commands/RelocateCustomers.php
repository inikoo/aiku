    ...
    public function handle() {
        ...
        if (Arr::get($tenant->data, 'legacy')) {
            ...
            foreach (DB::connection('legacy')->select("select * from".' '.$_table, []) as $legacy_data) {
                ...
                $billing_address->address_line_1 = $legacy_data->{'Customer Invoice Address Line 1'};
                $billing_address->address_line_2 = $legacy_data->{'Customer Invoice Address Line 2'};

                $checksum=$billing_address->checksum = $billing_address->checksum();


                new (Address)->updateOrCreate(
                    [
                        'checksum' => $checksum,
