...
class CallApcgbShipperApi
{
    ...
    public function handle(DeliveryNote $deliveryNote, Shipper $shipper, $type): Shipment
    {
        ...
        if ($type == 'apiCall') {
            $apiResponse =  $this->callApi($apiurl.'Orders.json',$header,$params);
        } else {
            $apiResponse =  $this->callApi($apiurl.'Tracks/'.$shipment->tracking.'.json?searchtype=CarrierWaybill&history=Yes',$header,$params);
        }
