        ...
        <div class="overflow-hidden rounded-lg bg-white shadow">
            <div class="divide-y divide-gray-200 lg:grid lg:grid-cols-12 lg:divide-y-0 lg:divide-x">
                ...
                <aside class="py-0 lg:col-span-3 lg:h-screen">
                    <div>
                        <h2 class="py-3 pl-2 font-bold leading-7 text-gray-900 sm:truncate lg:text-2xl sm:tracking-tight capitalize">
                            {{ pageHead.title }}
                        </h2>
                    </div>
                    <nav role="navigation" class="space-y-1">
                        <ul>
                            <li v-for="(item,key) in pageBody.layout" @click="current=key"
                                :class="[key===current ? 'bg-teal-50 border-teal-500 text-teal-700 hover:bg-teal-50 hover:text-teal-700' : 'border-transparent text-gray-900 hover:bg-gray-50 hover:text-gray-900', 'cursor-pointer group border-l-4 px-3 py-2 flex items-center text-sm font-medium']"
                                :aria-current="key===current ? 'page' : undefined">

                                <font-awesome-icon aria-hidden="true"
                                                   :class="[key===current ? 'text-teal-500 group-hover:text-teal-500' : 'text-gray-400 group-hover:text-gray-500', 'flex-shrink-0 -ml-1 mr-3 h-6 w-6']"
                                                   :icon=item.icon
                                />
                ...
                <form v-show="current==='profile'" class="divide-y divide-gray-200 lg:col-span-9" @submit.prevent=" profileForm.post('/profile')">

                    <div class="py-6 px-4 sm:p-6 lg:pb-8">
                        <div>
                            <h2 class="text-lg font-medium leading-6 text-gray-900">{{ pageBody.layout.profile.title }}</h2>
                            <p v-if="pageBody.layout.profile.notes" class="mt-1 text-sm text-gray-500">{{ pageBody.layout.profile.notes }}</p>
                        </div>

                        <div class="mt-6 flex flex-col lg:flex-row">
                            <div class="flex-grow space-y-6">
                                <div>
                                    <label for="username" class="block text-sm font-medium text-gray-700">{{ pageBody.layout.profile.fields.username.label }}</label>
                                    <div class="mt-1 flex rounded-md shadow-sm">
                                        <input v-model="profileForm.username" type="text" name="username" id="username" autocomplete="username"
                                ...
                                <div>
                                    <label for="about" class="block text-sm font-medium text-gray-700">{{ pageBody.layout.profile.fields.about.label }}</label>
                                    <div class="mt-1">
                                    ...
                                    <p class="mt-2 text-sm text-gray-500">{{ pageBody.layout.profile.fields.about.notes }}</p>
                                </div>
                            </div>


                            <div class="mt-6 flex-grow lg:mt-0 lg:ml-6 lg:flex-shrink-0 lg:flex-grow-0">
                                <p class="text-sm font-medium text-gray-700" aria-hidden="true">{{ pageBody.layout.profile.fields['photo'].label }}</p>
                                        ...
                                        <div class="ml-5 rounded-md shadow-sm">
                                            <div
                                                ...
                                                <label for="mobile-user-photo" class="pointer-events-none relative text-sm font-medium leading-4 text-gray-700">
                                                    ...
                                                    <span class="sr-only"> {{ pageBody.layout.profile.fields['photo'].info }}</span>
                                                </label>
                                                <input id="mobile-user-photo" name="user-photo" type="file" @input="avatarUploaded($event.target.files[0])"
                                ...
                                <div v-if="profileForm.errors.avatar" class="text-red-700">{{ profileForm.errors.avatar }}</div>
                            </div>
                        ...
                        <div v-if="profileForm.errors.email" class="text-red-700">{{ profileForm.errors.email }}</div>
                    ...
                    <div class="divide-y divide-gray-200 pt-6">
                        <div class="mt-4 flex justify-end py-4 px-4 sm:px-6">
                            <button v-if="profileForm.isDirty" type="button"
                            ...
                            <button type="submit"
                                    :class="[profileForm.isDirty ? 'bg-sky-700 text-white hover:bg-sky-800  focus:outline-none focus:ring-2 focus:ring-sky-500 focus:ring-offset-2' : 'bg-gray-100 text-gray-400', 'ml-5 inline-flex justify-center rounded-md border border-transparent  py-2 px-4 text-sm font-medium  shadow-sm ']"
                ...
                <form v-show="current==='password'" class="divide-y divide-gray-200 lg:col-span-9" @submit.prevent="passwordForm.patch('/profile')">

                    <div class="py-6 px-4 sm:p-6 lg:pb-8">
                        <div>
                            <h2 class="text-lg font-medium leading-6 text-gray-900">{{ pageBody.layout.password.title }}</h2>
                            <p v-if="pageBody.layout.password.notes" class="mt-1 text-sm text-gray-500">{{ pageBody.layout.password.notes }}</p>
                        </div>


                        <div class="mt-6 grid grid-cols-12 gap-6">
                            <div class="col-span-12">
                                <label for="email" class="block text-sm font-medium text-gray-700">{{ pageBody.layout.password.fields.password.label }}</label>
                                <div class="mt-1 flex rounded-md shadow-sm">
                                    <Password id="password" name="password" v-model="passwordForm.password"/>

                                </div>
                                <div v-if="passwordForm.errors.password" class="text-red-700">{{ passwordForm.errors.password }}</div>
                ...
                <div v-show="current==='workplaces'" class="divide-y divide-gray-200 lg:col-span-9">
