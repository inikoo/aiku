...
const labelValue = ref(props.data.name)
const linkValue = ref(props.data.href)

const handleBlur = (type) => {
   const set = {...props.data, name : labelValue.value, href :  linkValue.value}
   props.save(set,type)
  }
...
<div class="mt-5 flex gap-2">
  <div class="flex-1" style="width:87%;">
    <div class="flex shadow-sm ring-1 ring-inset ring-gray-300 focus-within:ring-2 focus-within:ring-inset focus-within:ring-indigo-600 sm:max-w-md">
      ...
      <input @blur="handleBlur('edit')" v-model="labelValue" type="text" class="block flex-1 border-0 bg-transparent py-1.5 pl-1 text-gray-900 placeholder:text-gray-400 focus:ring-0 sm:text-sm sm:leading-6" placeholder="label" />
    </div>
    <div class="flex shadow-sm ring-1 mt-1 ring-inset ring-gray-300 focus-within:ring-2 focus-within:ring-inset focus-within:ring-indigo-600 sm:max-w-md">
      ...
      <input @blur="handleBlur('edit')" v-model="linkValue" type="text" class="block flex-1 border-0 bg-transparent py-1.5 pl-1 text-gray-900 placeholder:text-gray-400 focus:ring-0 sm:text-sm sm:leading-6" placeholder="link" />
    </div>
  </div>
