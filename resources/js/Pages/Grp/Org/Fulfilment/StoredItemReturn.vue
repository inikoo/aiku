...
<template>
    ...
    <Modal :isOpen="openModal" @onClose="openModal = false">
        <div class="min-h-72 max-h-96 px-2 overflow-auto">
            <TableReturn :dataRoute="storedItemRoute.index" :saveRoute="storedItemRoute.store"
                @onClose="() => openModal = false" :descriptor="StoredItemReturnDescriptor"
                :beforeSubmit="submitDataStoredItem" :onFilterDatalist="onFilterDatalist">
                <template #column-quantity="{ data: dataColumn }">
                    <FieldEditableTable :data="dataColumn.columnData" fieldName="total_quantity"
                        placeholder="Enter pallet Quantity" type="number"
                        @input="(e) => dataColumn.columnData.total_quantity = e" :min="1"
