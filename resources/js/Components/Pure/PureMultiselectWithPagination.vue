...
const optionsList = ref<Table | null>(null)
...
<template>
    ...
    <!-- <div class="relative w-full text-gray-600 rounded-sm"> -->
        ...
        >
            ...
            <template #afterlist>
                <div v-if="optionsList?.meta?.current_page && optionsList?.data?.length && (optionsList?.links?.prev || optionsList?.links?.next)" class="flex justify-center border-t border-gray-300 gap-x-2 py-2 px-4 cursor-default">
                    <div 
                        @click="() => optionsList?.links?.prev ? fetchProductList(optionsList?.links?.prev) : false"
                        ...
                        :class="optionsList?.links?.prev ? 'cursor-pointer ' : 'opacity-0'"
                    >
                    ...
                    <div class="w-16">
                        <PureInputNumber
                            v-model="optionsList.meta.current_page"
                            @update:modelValue="(value) => fetchProductList(getUrlFetch({page: value}))"
                        />
                    </div>

                    <div 
                        @click="() => optionsList?.links?.next ? fetchProductList(optionsList?.links?.next) : false"
                        class="flex justify-center items-center py-1 px-2 text-gray-500 hover:text-gray-700 border border-transparent hover:border-gray-300 rounded-md"
                        :class="optionsList?.links?.next ? 'cursor-pointer ' : 'opacity-0'"
                    >
                        <FontAwesomeIcon icon='fal fa-chevron-right' class='' fixed-width aria-hidden='true' />
                    </div>

                    <!-- <div><Button v-if="optionsList?.links?.prev" label="Prev"/></div>
                    <div><Button v-if="optionsList?.links?.next" label="Next" /></div> -->
                </div>
            </template>
        </Multiselect>
    <!-- </div> -->
