...
<template>
	<div v-if="displayBreadcrumbs" class="md:pr-10 xl:pr-56">
		...
		<div class="grid grid-flow-col justify-end pr-2 space-x-2 text-sm text-gray-700 font-semibold">
        	<Link v-if="navigation.previous" :href="route(navigation.previous.route.name, [previousParams1, previousParams2])" class="rounded flex items-center pr-2 pl-4 py-0.5 space-x-2 opacity-70 hover:opacity-100 hover:cursor-pointer hover:text-indigo-900"
        	...
        	<Link v-if="navigation.next" :href="route(navigation.next.route.name, [nextParams1, nextParams2])" class="rounded flex items-center pr-4 pl-2 py-0.5 space-x-2 opacity-70 hover:opacity-100 hover:cursor-pointer hover:text-indigo-900"
