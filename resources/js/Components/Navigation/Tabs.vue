...
const tabIconClass = function (current,type,align) {
    let iconClass='-ml-0.5 h-5 w-5 ';
                ...
                <nav class="-mb-px flex grow space-x-6 ml-4" aria-label="Tabs">
                    <template  v-for="(tab,tabSlug) in navigation" :key="tabSlug" >
                        <button
                            ...
                            <FontAwesomeIcon v-if="tab.icon" :icon="tab.icon" :class="tabIconClass(tabSlug===currentTab,tab.type,tab.align)" aria-hidden="true"/>
                            <span v-if="tab.type!=='icon'" class="capitalize">{{ tab.title }}</span>
                        </button>
                    </template>
                </nav>
                <nav class="-mb-px flex flex-row-reverse  space-x-4 mr-4" aria-label="Secondary Tabs">
                    <template  v-for="(tab,tabSlug) in navigation" :key="tabSlug" >
                        <button
                            v-if="tab.align==='right'"
                            @click="[$emit('update:tab', tabSlug),changeTab(tabSlug)]"
                            ...
                            :aria-current="tabSlug===currentTab ? 'page' : undefined">
                            <FontAwesomeIcon :title="capitalizeFirstLetter(tab.title)"  v-if="tab.icon" :icon="tab.icon" :class="tabIconClass(tabSlug===currentTab,tab.type,tab.align)" aria-hidden="true"/>
                            <span v-if="tab.type!=='icon'" class="capitalize">{{ tab.title }}</span>
                        </button>
                    </template>
                </nav>
