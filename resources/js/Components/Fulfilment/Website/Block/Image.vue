...
const setImage = (imageData: any) => {
    if (activeImageIndex.value !== null) {
        ...
        while (images.length <= activeImageIndex.value) {
            images.push({
                source: null,
                link_data: null // Add placeholders to maintain structure
            });
        }

        // Update only the source field without touching other properties
        images[activeImageIndex.value] = {
            ...images[activeImageIndex.value],  // Preserve existing fields like link_data
            source: imageData.source // Update only the source field with the new image data
        };

        // Emit the updated modelValue
        emits("update:modelValue", props.web_block.layout.data.fieldValue.value);
        emits("autoSave");
    } else {
...
const onUpload = (uploadData: any) => {
    if (activeImageIndex.value !== null && uploadData.data && uploadData.data.length <= 1) {
        ...
        images[activeImageIndex.value].source = {
            ...images[activeImageIndex.value],  // Preserve existing fields like link_data
            ...uploadData.data[0]    // Update only the source field with the uploaded image data
        };
        console.log(props.web_block.layout.data.fieldValue.value)
        emits("update:modelValue", props.web_block.layout.data.fieldValue.value);
        emits("autoSave");
    } else {
...
<template>
    <!-- Loop over the number of slots based on layout_type -->
    <div v-if="web_block?.layout?.data?.fieldValue?.value?.images" class="flex flex-wrap">
        <div v-for="index in getImageSlots(web_block?.layout?.data?.fieldValue?.value?.layout_type)" :key="index"
            class="p-2"
            :class="getColumnWidthClass(web_block?.layout?.data?.fieldValue?.value?.layout_type, index - 1)">

            <!-- If image exists at this slot -->
            <div v-if="web_block?.layout?.data?.fieldValue?.value?.images?.[index - 1]?.source"
                ...
                <div v-if="isEditable" class="absolute top-2 right-2 flex space-x-2">
                    <Button :icon="['far', 'fa-pencil']" size="xs" @click="openImageGallery(index - 1)" />
                </div>
                <Image :src="web_block?.layout?.data?.fieldValue?.value?.images?.[index - 1]?.source"
            ...
            <div v-else-if="isEditable" class="p-5">
                <div type="button" @click="openImageGallery(index - 1)"
