...
const textActions = [
	...
	{ slug: "redo", icon: "far fa-redo-alt", active: "redo", label: "Redo" },
	{
		...
		icon: "fal fa-eraser",
		active: "clear",
		label: "Clear style on selected text",
		class: "text-red-500",
	},
...
<template>
    ...
    <bubble-menu :editor="editorInstance" v-if="editorInstance">
		<div
			class="p-2 flex items-center gap-x-1 gap-y-1.5 flex-wrap border-b border-gray-500 bg-slate-300 rounded-md"
			v-if="editorInstance">
			<!-- Action: Heading -->
     
			<div class="group relative inline-block">
				<div
					class="text-xs min-w-16 p-1 appearance-none rounded cursor-pointer border border-gray-500"
					:class="{
						'bg-slate-700 text-white font-bold': editorInstance.isActive('heading'),
					}">
				...
				<div
					...
					<div
						...
						:class="{
							'bg-slate-700 text-white hover:bg-slate-700': editorInstance.isActive(
								'heading',
								{ level: index }
							),
						}"
						...
						role="button">
						<Teleport
							v-if="editorInstance.isActive('heading', { level: index })"
							to="#headingIndex"
							>{{ index }}</Teleport
						>
						H{{ index }}
			...
			<template v-for="action in textActions" :key="action.slug">
				...
				<ColorPicker
					...
					<FontAwesomeIcon
						...
						aria-hidden="true"
						:style="{
							color: editorInstance?.getAttributes('textStyle').color || '#010101',
						}" />
				</ColorPicker>
				...
				<ColorPicker
					...
					@changeColor="
						(color) => editorInstance?.chain().setHighlight({ color: color.hex }).run()
					"
					class="flex items-center justify-center w-6 aspect-square rounded cursor-pointer border border-gray-700"
				...
				<div v-else-if="action.slug === 'fontsize'" class="group relative inline-block">
					<div
						class="flex items-center text-xs min-w-10 py-1 pl-1.5 pr-0 appearance-none rounded cursor-pointer border border-gray-500"
						:class="{
				...
				<div
					...
					:class="[
						action.class,
						{ 'bg-slate-700 text-white': editorInstance.isActive(action.active) },
					]">
					<FontAwesomeIcon
						v-if="action.icon"
						:icon="action.icon"
		...
		<div
			v-if="editorInstance && fieldData?.maxLength"
			class="text-gray-500 text-sm text-right p-1.5">
			<span :class="fieldData?.maxLength ? limitWarning : ''">
				{{ charactersCount }}
				{{ fieldData?.maxLength ? `/ ${fieldData?.maxLength} characters` : "characters" }}
			</span>
			|
			<span> {{ wordsCount }} {{ wordsCount > 1 ? "words" : "word" }} </span>
		</div>
	</div>
