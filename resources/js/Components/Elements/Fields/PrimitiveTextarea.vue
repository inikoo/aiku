...
const value1 = ref(props.modelValue ? props.modelValue : '')
</script>
...
<template>
    <div>
        ...
        <div class="mt-1  rounded-md shadow-sm">
            <textarea id="about" name="about" rows="3" v-model.trim="value1" @input="$emit('update:modelValue', value1)" placeholder="Tell us about yourself"
        ...
        <div v-if="showStats" class="grid grid-flow-col text-xs italic text-gray-500 mt-2 space-x-12 justify-start">
            <p class="">
                ...
                Letters: {{ value1.length }}
            </p>
            <p class="">
                <!-- {{ pageBody.layout.profile.fields.about.notes }} -->
                Words: {{ value1.trim().split(/\s+/).filter(Boolean).length }}
            </p>
        </div>
    </div>
</template>
