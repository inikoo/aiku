...
<template>
    <component
        :is="props.route ? Link : 'button'"
        @start="() => isLoadingVisit = true"
        @finish="() => isLoadingVisit = false"
        :method="props.route?.method || undefined"
        :data="props.route?.body"
        type="button"
        ...
        :disabled="isLoadingVisit || loading || disabled || style == 'disabled' || type == 'disabled'"
        v-tooltip="tooltip ?? undefined"
    >
        <slot>
            <slot name="loading">
                <FontAwesomeIcon v-if="isLoadingVisit || loading" icon='fad fa-spinner-third' class='animate-spin' fixed-width  aria-hidden="true"/>
            </slot>
            <slot name="icon" v-if="!loading && !isLoadingVisit">
