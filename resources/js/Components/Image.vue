...
<template>
    <picture :class="[props.class ?? 'w-full h-full flex justify-center items-center']">
        ...
        <img :class="[imageCover ? 'w-full object-cover aspect-auto' : '']" :style="{height: 'inherit', ...style }" @load="() => emits('onLoadImage')" :srcset="original" :src="get(src, 'original')" :alt="alt"  v-bind="imgAttributes">
    </picture>
</template>
