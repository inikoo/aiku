<template>
    <div class="w-full min-w-max bg-gray-800 shadow-lg flex-row items-start text-gray-800 text-xs cursor-default" >
        <div class="flex flex-col justify-center text-center p-1 pb-3 gap-y-1">
            <div v-for="(option, index) in data" class="flex justify-start py-1 px-2 gap-x-1.5 hover:bg-gray-700">
                ...
                <p class="text-left text-gray-100">
                    ...
                    <span class="text-gray-300"> ({{ option.user.username }})</span> - 
                    <span class="capitalize">{{ option.route.module }}</span>
                </p>
            </div>
        </div>
    </div>
