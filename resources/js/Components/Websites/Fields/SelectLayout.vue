...
const tempSelectedLayout = ref<Layout | null>(null) 
...
const selectLayout = (layoutType: string) => {
	tempSelectedLayout.value =
		layouts.value.find((layout) => layout.layout_type === layoutType) || null 
}

const onSubmitNote = async () => {
	if (tempSelectedLayout.value !== null) {
		emit("update:modelValue", {
			...props.modelValue,
			layout_type: tempSelectedLayout.value.layout_type,
		})
	...
	<Modal :isOpen="isModalOpen" @onClose="() => (isModalOpen = false)">
		<div class="h-auto px-2 overflow-auto">
			...
			<div class="relative">
				<div class="grid grid-cols-3 gap-4">
					<div
						...
						:key="layout.layout_type"
						class="p-2 cursor-pointer transition-all"
						:class="{
							'border-blue-500 ring-2 rounded':
								tempSelectedLayout &&
								tempSelectedLayout.layout_type === layout.layout_type,
							'hover:border-blue-300 hover:bg-gray-100':
								!tempSelectedLayout ||
								tempSelectedLayout.layout_type !== layout.layout_type,
						}"
						@click="selectLayout(layout.layout_type)">
						<div class="border rounded-lg p-4">
							...
							<div v-else class="flex gap-2">
								<div
									v-for="(flexClass, index) in layout.flex"
									:key="index"
									:class="flexClass"
									class="p-2 bg-gray-200 h-20 text-center border border-dashed rounded-lg">
									<font-awesome-icon
			...
			<div class="flex justify-end gap-x-2 mt-3">
				...
				<Button
					label="Submit"
					:disabled="!tempSelectedLayout"
					@click="onSubmitNote"
					class="transition-colors"
					:class="{
						'bg-gray-300': !tempSelectedLayout,
						'bg-blue-500 hover:bg-blue-600': tempSelectedLayout,
					}" />
			</div>
		</div>
	</Modal>
