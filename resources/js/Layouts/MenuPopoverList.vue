    ...
    <div class="px-1 py-1">
        ...
        <div class="max-h-52 overflow-y-auto space-y-1.5">
            <MenuItem
                ...
                v-slot="{ active }"
                v-tooltip="showare.state == 'closed' ? `This ${navKey} is closed` : false"
                as="div"
                :disabled="showare.state == 'closed'"
                @click="() => router.visit(route(showare.route?.name, showare.route?.parameters))" :class="[
                    showare.state == 'closed' ? 'bg-slate-200 select-none curs' : showare.slug == layout.currentParams[navKey] ? 'bg-slate-500 text-white cursor-pointer' : 'text-slate-600 hover:bg-slate-200/75 hover:text-indigo-600 cursor-pointer',
