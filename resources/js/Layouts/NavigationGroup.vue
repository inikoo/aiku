...
<template>
    <Disclosure v-slot="{ open }">
        ...
        >
            ...
            <DisclosureButton v-slot="{ open }" class="w-full flex justify-between items-end pt-2 px-2.5 pb-2 text-indigo-100/70">
                ...
                <FontAwesomeIcon icon='fal fa-chevron-down' class='justify-self-end text-xs transition-all duration-200 ease-in-out' :class="[open ? 'rotate-180' : '']" aria-hidden='true' />
            </DisclosureButton>
            ...
            <DisclosurePanel v-if="Object.keys(orgNav || []).length === 1" class="flex flex-col gap-y-1 mb-1">
                <!-- group only 1 -->
            ...
            <DisclosurePanel v-else-if="layout.organisationsState?.[layout.currentParams.organisation]?.[generateCurrentString(itemKey)]"
