...
// console.log(Object.keys(layout.groupNavigation).length)
...
const handleKey = (event: any) => {
    ...
    if (event.key === "ArrowUp" && isHover.value && currentIndexModule.value != 0) {
        const prevTab = ref(layout.groupNavigation[Object.keys(layout.groupNavigation)[currentIndexModule.value - 1]])
    ...
    else if (event.key === "ArrowDown" && isHover.value && currentIndexModule.value != Object.keys(layout.groupNavigation).length - 1) {
        const nextTab = ref(layout.groupNavigation[Object.keys(layout.groupNavigation)[currentIndexModule.value + 1]])
...
console.log(layout.navigation.grp)
</script>
...
<template>
    ...
    <div class="flex flex-grow flex-col pb-16">
        <nav class="flex-1 space-y-1" aria-label="Sidebar">
            ...
            <!-- LeftSide Links -->
            <Link v-for="(item, itemKey) in layout.navigation.grp" :key="itemKey" :href="generateRoute(item)" :class="[
                ...
                <div>
                    <img v-if="item.name == 'dashboard'" src="@/../art/logo/png/logo-aiku.png" alt="Aiku Logo"
                        class="h-4 aspect-square" :class="[itemKey === layout.currentModule
                            ? 'text-indigo-500'
                            : 'text-gray-400 group-hover:text-gray-600',
                            'ml-2 mr-3 flex-shrink-0 h-4 w-4'
                        ]">
                    <FontAwesomeIcon v-else aria-hidden="true" :class="[
                        itemKey === layout.currentModule
                            ? 'text-indigo-500'
                            : 'text-gray-400 group-hover:text-gray-600',
                        'ml-2 mr-3 flex-shrink-0 h-4 w-4',
