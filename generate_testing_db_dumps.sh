...
pg_dump -Fc -p "${DB_PORT}" -f "tests/datasets/db_dumps/d1_fresh_with_assets.dump" ${DB}
...
pg_dump -Fc -p "${DB_PORT}" -f "tests/datasets/db_dumps/d2_with_admin.dump" ${DB}
...
pg_dump -Fc -p "${DB_PORT}" -f "tests/datasets/db_dumps/d3_with_tenants.dump" ${DB}
